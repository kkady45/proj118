...
function draw() {
    ...
    if (!started) {
        ctx.arc(0.5*canvas.width+0.005*distance*canvas.width, canvas.height/2, 0.005*circsize*canvas.height, 0, 2*Math.PI);
        ctx.arc(0.5*canvas.width-0.005*distance*canvas.width, canvas.height/2, 0.005*circsize*canvas.height, 0, 2*Math.PI);
        ctx.fill();
        return;
    }
    ...
    if (inTransition && Date.now() - interframeTimer < interframeInterval) {
        ctx.fill();
        return;
    }
