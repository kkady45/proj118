...
const comparisonValues = [102.9, 205.8, 308.7, 411.6, 514.5, 617.4];
const jndValues = [2.9, 5.8, 8.7, 11.6, 14.5, 17.4];
const deltaI_I = [2.9, 5.8, 8.7, 11.6, 14.5, 17.4]; 
const yMax = 24; // Maximale Höhe für die y-Achse
...
function updateGraph() {
    ...
    for (let i = 0; i <= currentMaxIndex; i++) {
        ...
        const y = canvas.height - 50 - (jndValues[i] / yMax) * (canvas.height - 100);
        points.push({ x, y });
    }
    drawGraph();
}
...
function drawGraph() {
    ...
    // Linie zeichnen, wenn der letzte Button geklickt wurde
